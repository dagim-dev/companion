...
from memory_extraction_jobs import (
    STATUS_PENDING,
...
def _process_claimed_job(user_id: str, job: dict) -> bool:
    """Run extraction for an already-claimed job. Always returns True."""
    try:
...
def process_next_available_job() -> bool:
    ...
    if claimed is None:
        return False
    user_id, job = claimed
    return _process_claimed_job(user_id, job)
